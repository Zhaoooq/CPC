...
void TouchDoubleSpinBox::openNumericKeypad() {
    ...
    title->setStyleSheet("font-size: 24px; font-weight: bold; color: #2C5D7C;");
    ...
    rangeHint->setStyleSheet("font-size: 17px; color: #607481;");
        ...
        if (!ok || !std::isfinite(entered) || entered < minimum() || entered > maximum()) {
            rangeHint->setText(
                QString("请输入 %1 ～ %2 范围内的数值")
                    .arg(minimum(), 0, 'f', decimals())
                    .arg(maximum(), 0, 'f', decimals()));
            rangeHint->setStyleSheet("font-size: 17px; color: #C0392B; font-weight: bold;");
            return;
        }
        setValue(entered);
        dialog.accept();
