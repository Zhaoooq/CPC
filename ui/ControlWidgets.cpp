...
QGroupBox* createTempGroup(
    ...
    btnStart->setStyleSheet(QString(
        "QPushButton { background-color: %1; color: white; border: none; border-radius: 7px; font-size: 18px; font-weight: bold; }"
    ...
    btnStop->setStyleSheet(
        "QPushButton { background-color: #7F8C8D; color: white; border: none; border-radius: 7px; font-size: 18px; font-weight: bold; }"
    ...
    targetLabel->setStyleSheet("font-size: 17px; color: #566573;");
    lblTemp = new QLabel("当前: -- ℃");
    lblTemp->setAlignment(Qt::AlignCenter);
    lblTemp->setStyleSheet(QString("font-size: 27px; font-weight: bold; color: %1; background: #F7FAFC; border-radius: 7px; padding: 7px;").arg(colorHex));
    lblPwm = new QLabel("功率: -- %");
    lblPwm->setAlignment(Qt::AlignCenter);
    lblPwm->setStyleSheet("font-size: 17px; color: #667884;");
...
QLabel* createOverviewValue(const QString& text, const QString& colorHex) {
    ...
    label->setStyleSheet(QString("font-size: 24px; font-weight: bold; color: %1;").arg(colorHex));
    return label;
}
