...
| 页面 | 主要内容 |
| --- | --- |
| 总览 | 颗粒数浓度、浓度趋势、温度状态灯、液位状态、气泵/风扇/旁路/压差摘要、采集启停和原始数据保存 |
| 温控 | 冷凝段与饱和段目标温度、温控启停、实时温度和 PWM 功率；OPC 段温度与加热输出；OPC 风扇和旁路流量模式 |
| 气路 | 气泵功率、比例阀手动开度、4–20 mA 输出读取/关闭、压差闭环设置、三路 ADS1115 压差监测与重新校零 |
...
| GPIO5 | 29 | OPC 风扇 | 数字输出 | 高电平开启，低电平关闭；默认关闭 |
| GPIO24 | 18 | 旁路电磁阀 | 数字输出 | 高电平打开=1.5 L/min；低电平关闭=0.3 L/min，默认为小流量 |
| GPIO6 | 31 | OPC 段加热输出 | `lgpio` PWM，5 Hz | 当前界面使用 100%/0% 启停；默认关闭 |
...
程序初始化数字输出时默认写低电平。正常退出或点击右上角关机按钮后，会依次：

1. 停止液位监控并关闭进液/排液阀。
2. 停止 ADS1115 压差采集。
3. 尝试把比例阀输出设为 4.00 mA，然后关闭串口。
4. 将气泵、OPC 风扇、旁路阀、制冷片、饱和段加热和 OPC 加热输出全部置零/低电平。
5. 停止 FTDI 数据采集线程并释放 GPIO。
